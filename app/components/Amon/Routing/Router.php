...
class Router extends Injectable
{
	...
	public function addRoutes($routes){
		...
		foreach($routes as $route) {
			...
			if (!isset($route['titles'])) {
				$route['titles'] = [];
			}
			$this->map($route['url'], $methods, $route['name'], $route['active'], $route['titles']);
		}
	}

	protected function map($url, $methods, $name, $active, $titles) {
		$route['name'] = $name;
		$route['url'] = $url;
		...
		if (class_exists($this->getController())) {
			...
			if ( method_exists($controller, $this->getAction()) ){
				if ( method_exists($controller, 'beforeExecuteRoute') ){
					$params['before_action'] = $this->getAction();
					call_user_func_array([$controller, 'beforeExecuteRoute'], [$params]);
					unset($params['before_action']);
				}
				call_user_func_array([$controller, $this->getAction()], [$params]);
				if ( method_exists($controller, 'afterExecuteRoute') ){
					$params['after_action'] = $this->getAction();
					call_user_func_array([$controller, 'afterExecuteRoute'], [$params]);
					unset($params['after_action']);
				}
			}else{
